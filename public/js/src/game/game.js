define([
  ...
  var Game = function Game(nsp, pid) {
    this.nsp     = nsp;
    this.pid     = pid;
    this.units   = [];
    this.planets = [];
  ...
  Game.prototype.detectEnd = function(clock) {
    var _this = this;
    if (this.planets.filter(function(p) { return p.owner === _this.pid; }).length === 0) {
      this.end(clock, false);
    }
    if (this.planets.filter(function(p) { return p.owner !== null && p.owner !== _this.pid; }).length === 0) {
      this.end(clock, true);
    }
  };
